/*!
 *  QuickDegreeFinder.jquery.js
 */

/*
    Provide functional, objectified & parameterized API that
...
;var QuickDegreeFinder = function(settings) {
    ...
    // Default options
    var _options = {
        dataUrl:    ('http:' === window.location.protocol ? 'http://static' : 'https://schools') + '.collegedegrees.com/js/widget.json',
        defaults:   _getStruct(),
        elements:   _getStruct(),
        excludes:   _getStruct(),
        limits:     _getStruct(),
    ...
    /**
     *  Widget data for QuickDegreeFinder
     *
     *  @return {Object}
     */
    ...
    this.loadData = function(url) {
        var callback = 'QuickDegreeFinder.instances[' + QuickDegreeFinder.instances.indexOf(this) + '].setData';

        // jQuery appends =TIMESTAMP to script URLs for caching,
        // which results in a 404.
        var script = document.createElement('script');
        script.src = url || this.getOptions('dataUrl') + '?callback=' + callback;
    ...
    QuickDegreeFinder.instances.push(this);
    ...
    /**
     *  Expose public methods as QuickDegreeFinder API
     */
    return {
